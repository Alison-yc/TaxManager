...
export function UserImportMenuItem() {
  ...
  async function handleFile(ev: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      if (pendingKind === 'excel') {
        const { uploadFormDataFromExcelFile } = await import('../lib/formDataExcelUpload')
      ...
      if (pendingKind === 'invoice-pdf') {
        const { uploadInvoicePdfFile } = await import('../lib/pdfImport/declarationPdfImport')
      ...
      if (pendingKind === 'tax-payment-cert-pdf') {
        const { uploadTaxPaymentCertPdfFile } = await import('../lib/pdfImport/taxPaymentCertPdfImport')
      ...
      const { uploadDeclarationPdfFile } = await import('../lib/pdfImport/declarationPdfImport')
      const category = pendingKind === 'financial-pdf' ? 'financial' : 'declaration'
      const r = await uploadDeclarationPdfFile(file, category)
      if (r.ok === false) {
        void message.error(r.message)
        return
      }
      void message.success(category === 'financial' ? '财务报表 PDF 导入成功' : '申报 PDF 导入成功')
  ...
  const menuItems: MenuProps['items'] = [
    { key: 'excel', label: '导入 Excel 申报表', onClick: () => openPicker('excel') },
    { key: 'invoice-pdf', label: '导入 PDF 发票', onClick: () => openPicker('invoice-pdf') },
    {
  ...
  return (
    <>
      <div className="etax-user-excel-slot">
        <Dropdown menu={{ items: menuItems }} trigger={['click']} disabled={busy}>
          <button
            type="button"
