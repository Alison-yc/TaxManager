...
function invoiceRiskTagClass(level?: string | null): string {
  ...
  if (v === "正常")
    return "etax-invoice-risk-tag etax-invoice-risk-tag--normal";
  if (v.includes("疑") || v.includes("风险"))
    return "etax-invoice-risk-tag etax-invoice-risk-tag--warn";
        ...
        render: (_v, row) => (
          <Space size={8}>
            <Link
              ...
              to={`/invoice-query/record/${row.id}`}
            >
              详情
            </Link>
            <Link
              className="etax-q-table-link"
              to={`/invoice-query/record/${row.id}?preview=1`}
            >
              预览
            </Link>
            <button
              type="button"
              className="etax-q-table-link etax-q-table-link-btn"
              disabled
            >
              交付
            </button>
      ...
      <div className="etax-query-page etax-invoice-query-page etax-query-antd">
        <div className="etax-query-bc-bar">
          <button
            ...
            onClick={() => navigate("/invoice-query/stats")}
          >
            ← 返回
          </button>
            ...
            >
              ...
              <div
                className={
                  filtersExpanded
                    ? undefined
                    : "etax-query-filters-extra--collapsed"
                }
              >
                <Row gutter={[28, 18]}>
                  <Col xs={24} lg={8}>
          ...
          <Card className="etax-query-result-card">
            <div className="etax-invoice-query-toolbar">
              ...
              <div className="etax-invoice-query-summary">
                查询结果：合计金额：{fmtMoney(totals.amount)}元 合计税额：
                {fmtMoney(totals.tax)}元
                {rows.length >= INVOICE_QUERY_DISPLAY_LIMIT ? (
                  <span className="etax-invoice-query-summary-hint">
                    （列表最多显示 {INVOICE_QUERY_DISPLAY_LIMIT}{" "}
                    条，导出全部可获取完整数据）
                  </span>
                ) : null}
              </div>
