...
export function applyInvoiceRecordFilters(
  ...
  if (filters.invoiceSource !== '全部') q = q.eq('invoice_source', filters.invoiceSource)
  ...
  if (filters.invoiceType !== '全部') q = q.eq('invoice_type', filters.invoiceType)
  ...
  if (filters.issueFrom) q = q.gte('issue_date', filters.issueFrom.startOf('day').toISOString())
  if (filters.issueTo) q = q.lte('issue_date', filters.issueTo.endOf('day').toISOString())
  return q
}
